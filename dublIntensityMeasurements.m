...
%    population-level intensity analyses for an experiment. Works for
%    initCoord data, i.e. unpaired movies. Options are available.
%
%    Options, defaults in {}:-
%
...
%
%
% Copyright (c) 2018 C. A. Smith, modified August 2023 by C. C. Conway

% default options
opts.category = [];
opts.channels = [1 2];
opts.refMarker = 'inner';
opts.paired = 0;
opts.prevMeas = [];
opts.spotSelection = [];
% user options
...
for iExpt = 1:numExpts
    ...
    for iMov = 1:length(theseMovies)
      ...
      if isfield(theseMovies{iMov}, 'keep')
        if ~theseMovies{iMov}.keep
            noDS = [noDS; iExpt movNum];
            continue
        end
      end
