...
% Modified by C. C. Conway and A. Daniyan (2022)
...
function newRunCB(hObj,event)
  ...
  jobset = kitSetupJob; %AD and CCC edits on kitSetupJob
...
function editRunCB(hObj,event)
  ...
  jobset = kitSetupJob(hs.jobset); %AD and CCC edit on kitSetupJob
...
function executeCB(hObj,event)
  ...
  % Run the job.
  progh = waitbar(1/length(handles.jobset.ROI),sprintf('Tracking progress (%d/%d)',1,length(handles.jobset.ROI))); %CCC addition
  kitRunJob(handles.jobset,'callback',@trackProgress);%,'tasks',handles.jobset.options.tasks); %CCC edit on kitRunJob
  delete(progh);
...
function trackProgress(idx)
  waitbar((idx+1)/length(handles.jobset.ROI),progh,sprintf('Tracking progress (%d/%d)',(idx+1),length(handles.jobset.ROI)));
end
