function kitManualPairSisters(jobset,varargin)
% KITMANUALPAIRSISTERS Manually select pairs of kinetochores.
%
%    JOB = KITMANUALPAIRSISTERS(JOB,...) Images of each kinetochore found
...
% Modified by A. Daniyan November 2022

% default options
opts.chanOrder = [2 1 3]; % order of the channels in the movie
opts.contrast = {[0.1 0.9995] [0.1 0.9995] [0.1 1]};
opts.imageChans = 2; % imaging more channels may provide more information %A. Daniyan edited from 1 to 2 21/11/2022
opts.maxSisSep = 2.5; % maximum distance in µm between potential sisters being plotted
opts.mode = 'dual';
opts.plotChan = 2; % channel coordinates to be plotted for allocation %A. Daniyan edited from 1 to 2 21/11/2022
opts.redo = 1; %A. Daniyan edited from 0 to 1 21/11/2022
opts.subset = [];
opts.verbose = 0;
opts.zProjRange = 2; % number of pixels over which to project in z
opts.autoSize = 0; %Addition A. Daniyan 21/11/2022
...
%     else
      [jobs{iMov},userStatus] = pairSpots(jobs{iMov},opts); %Function change A. Daniyan 21/11/2022
%     end
