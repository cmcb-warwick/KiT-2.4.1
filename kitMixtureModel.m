...
debug(frames) = struct('spotsNoRef',[],'spotRef',[],'rejAmp',[],'rejDist',[]);  % A.D. addition to compare spots before and after mmf 07/02/2023
...
for iImage = goodImages
  ...
  opts.mmf.alphaA = max(opts.mmf.alphaA);% This allows a higher threshold for spot rejection during amplitude F-testing. A.D. edit mmf 07/02/2023
...
%     title('MMF fitted particles (r), cands (b), amp rej (g), dist rej (y)');
    title(['Frame: ' num2str(iImage) ', Pre MMF: ' num2str(size(cands,1)) ',' ...
        ' Post MMF: ' num2str(size(coordList,1)) ',\newlineMMF fitted particles (r),' ...
        ' cands (b), amp rej (g), dist rej (y)'], 'FontSize',14,'FontWeight','bold','HorizontalAlignment','center');
    hold off;
    drawnow;
