...
% Modified by C.C. Conway and A. Daniyan November 2022
...
function hs = createControls()
  ...
  colw = [55 50];
  figw = sum(colw);
  figh = 53; %CCC edit from 49 2022/06/20
  figpos = [100 20 figw figh]; %CCC edit 35 to 20 2022/06/20 
  ...
  hs.ROIs.Min = 1;
  hs.ROIs.Max = inf; %CCC 2023.08.09
  % undocumentedmatlab.com hack to add horizontal scrollbars
  ...
  % Create options tabs.
  panw = colw(2)-2*dx;
  panh = 15.5*h;%CCC edit from 13.5*h 2022/07/19
  panx = colw(1)+dx;
  pany = y-panh;
  ...
  if ~strcmp(hs.mode,'chrshift')
      ...
      hs.cs_ch1to2Vect = label(p,sprintf('1  ->  2'),[labx cspany labw h],tinyfont);
      hs.cs_ch1to2Arrow = label(p,'->',[(editx1+editx2)/2 cspany editw h],tinyfont);
      hs.cs_ch1to2Arrow.HorizontalAlignment = 'center';
      hs.cs_ch1to2Jobset = button(p,'-',[btnx cspany btnw h],@ch1to2CB,tinyfont);
      hs.cs_ch1to2_ch1numOrder = editbox(p,[],[editx1 cspany editw h],tinyfont);
      hs.cs_ch1to2_ch2numOrder = editbox(p,[],[editx2 cspany editw h],tinyfont);
      cspany = cspany-h;

      hs.cs_ch1to3Vect = label(p,sprintf('1  ->  3'),[labx cspany labw h],tinyfont);
      hs.cs_ch1to3Arrow = label(p,'->',[(editx1+editx2)/2 cspany editw h],tinyfont);
      hs.cs_ch1to3Arrow.HorizontalAlignment = 'center';
      hs.cs_ch1to3Jobset = button(p,'-',[btnx cspany btnw h],@ch1to3CB,tinyfont);
      hs.cs_ch1to3_ch1numOrder = editbox(p,[],[editx1 cspany editw h],tinyfont);
      hs.cs_ch1to3_ch3numOrder = editbox(p,[],[editx2 cspany editw h],tinyfont);
      cspany = cspany-h;
      % if ch 2 -> 3 exists 
      hs.cs_ch2to3Vect = label(p,sprintf('2  ->  3'),[labx cspany labw h],tinyfont);
      hs.cs_ch2to3Arrow = label(p,'->',[(editx1+editx2)/2 cspany editw h],tinyfont);
      hs.cs_ch2to3Arrow.HorizontalAlignment = 'center';
      hs.cs_ch2to3Jobset = button(p,'-',[btnx cspany btnw h],@ch2to3CB,tinyfont);
      hs.cs_ch2to3_ch2numOrder = editbox(p,[],[editx1 cspany editw h],tinyfont);
      hs.cs_ch2to3_ch3numOrder = editbox(p,[],[editx2 cspany editw h],tinyfont);
      cspany = cspany-h;
...
%       end
  ...
  t.FontWeight = 'bold';
  y = y-h;
  labw = 17; %CCC edit from 15 2022/07/19
  hs.filenameTxt = label(hs.fig,['kitjobset_' datestr(now,'yymmdd') '_'],[labx y labw h],smallfont);
  hs.filename = editbox(hs.fig,'filename',[labx+labw y w-(labw+ddx) h]); %CCC edit from labx+(labw+ddx) 2022/07/19
%   y = y-h;
%   tickw = w;
%   hs.groupOutput = checkbox(hs.fig,'Group output files into separate folder',[labx y tickw h],[],tinyfont);
  y = y-lh;
  btnw = 17.5;
  btnx = labx+(w/2-btnw/2);
  btnh = 2;
...
%   hs.groupOutput.Value = opts.groupOutput;
  ...
  if ~strcmp(hs.mode,'chrshift')
    ...
    if ~isempty(opts.chrShift.jobset{1,2})
        hs.cs_ch1to2Jobset.String = opts.chrShift.jobset{1,2};
        hs.cs_ch1to2_ch1numOrder.Enable = 'on';
        hs.cs_ch1to2_ch1numOrder.String = num2str(opts.chrShift.chanOrder{1,2}(1));
        hs.cs_ch1to2_ch2numOrder.Enable = 'on';
        hs.cs_ch1to2_ch2numOrder.String = num2str(opts.chrShift.chanOrder{1,2}(2));
    else
        hs.cs_ch1to2Jobset.String = '-';
        hs.cs_ch1to2_ch1numOrder.String = '';
        hs.cs_ch1to2_ch1numOrder.Enable = 'off';
        hs.cs_ch1to2_ch2numOrder.String = '';
        hs.cs_ch1to2_ch2numOrder.Enable = 'off';
    end

    if ~isempty(opts.chrShift.jobset{1,3})
        hs.cs_ch1to3Jobset.String = opts.chrShift.jobset{1,3};
        hs.cs_ch1to3_ch1numOrder.Enable = 'on';
        hs.cs_ch1to3_ch1numOrder.String = num2str(opts.chrShift.chanOrder{1,3}(1));
        hs.cs_ch1to3_ch3numOrder.Enable = 'on';
        hs.cs_ch1to3_ch3numOrder.String = num2str(opts.chrShift.chanOrder{1,3}(2));
    else
        hs.cs_ch1to3Jobset.String = '-';
        hs.cs_ch1to3_ch1numOrder.String = '';
        hs.cs_ch1to3_ch1numOrder.Enable = 'off';
        hs.cs_ch1to3_ch3numOrder.String = '';
        hs.cs_ch1to3_ch3numOrder.Enable = 'off';
    end
%         if ch 2 -> 3 exist
    if ~isempty(opts.chrShift.jobset{2,3})
        hs.cs_ch2to3Jobset.String = opts.chrShift.jobset{2,3};
        hs.cs_ch2to3_ch2numOrder.Enable = 'on';
        hs.cs_ch2to3_ch2numOrder.String = num2str(opts.chrShift.chanOrder{2,3}(1));
        hs.cs_ch2to3_ch3numOrder.Enable = 'on';
        hs.cs_ch2to3_ch3numOrder.String = num2str(opts.chrShift.chanOrder{2,3}(2));
    else
        hs.cs_ch2to3Jobset.String = '-';
        hs.cs_ch2to3_ch2numOrder.String = '';
        hs.cs_ch2to3_ch2numOrder.Enable = 'off';
        hs.cs_ch2to3_ch3numOrder.String = '';
        hs.cs_ch2to3_ch3numOrder.Enable = 'off';
    end
...
function cropROICB(hObj,event)
  ...
  % Start waitbar.
  waitmsg = sprintf('Cropped %d of %d', 0, length(v));
  hwait = waitbar(0,waitmsg); %CCC removed for Sonoma freezing 2024.02.19
  kitLog(waitmsg); %CCC addition to allow viewing of progress 2024.02.19
  ...
  for i=1:length(v)
    ...
    waitmsg = sprintf('Cropped %d of %d', i, length(v));
    kitLog(waitmsg); %CCC addition to allow viewing of progress 2024.02.19
    waitbar(i/length(v),hwait,waitmsg); %CCC removed for Sonoma freezing 2024.02.19
  end
  populateROIBox();
  close(hwait); %CCC removed for Sonoma freezing 2024.02.19
end
...
if ~strcmp(handles.mode,'chrshift') && any(strcmp(mapStrings(handles.coordSys.Value,coordSystemValues),{'None'})) %CCC edited removed {'Centre of mass'} as additional restriction 2022/10/06
    handles.neighbourMaskShapeText.Enable = 'off';
...
function chromaticShiftCB(hObj,event)
  if handles.chromaticShift.Value
    ...
    handles.cs_ch1to2Vect.Enable = 'on';
    handles.cs_ch1to2Arrow.Enable = 'on';
    handles.cs_ch1to2Jobset.Enable = 'on';

    handles.cs_ch1to3Vect.Enable = 'on';
    handles.cs_ch1to3Arrow.Enable = 'on';
    handles.cs_ch1to3Jobset.Enable = 'on';

    handles.cs_ch2to3Vect.Enable = 'on';
    handles.cs_ch2to3Arrow.Enable = 'on';
    handles.cs_ch2to3Jobset.Enable = 'on';

    if all(~strcmp(handles.cs_ch1to2Jobset.String,{'-','Unknown source'}))
        handles.cs_ch1to2_ch1numOrder.Enable = 'on';
        handles.cs_ch1to2_ch2numOrder.Enable = 'on';
        handles.cs_ch1to2_ch1numOrder.String = '1';
        handles.cs_ch1to2_ch2numOrder.String = num2str(1+1);
    else
        handles.cs_ch1to2_ch1numOrder.Enable = 'off';
        handles.cs_ch1to2_ch2numOrder.Enable = 'off';
        handles.cs_ch1to2_ch1numOrder.String = '';
        handles.cs_ch1to2_ch2numOrder.String = '';
    end

    if all(~strcmp(handles.cs_ch1to3Jobset.String,{'-','Unknown source'}))
        handles.cs_ch1to3_ch1numOrder.Enable = 'on';
        handles.cs_ch1to3_ch3numOrder.Enable = 'on';
        handles.cs_ch1to3_ch1numOrder.String = '1';
        handles.cs_ch1to3_ch3numOrder.String = num2str(2+1);
    else
        handles.cs_ch1to3_ch1numOrder.Enable = 'off';
        handles.cs_ch1to3_ch3numOrder.Enable = 'off';
        handles.cs_ch1to3_ch1numOrder.String = '';
        handles.cs_ch1to3_ch3numOrder.String = '';
    end

    if all(~strcmp(handles.cs_ch2to3Jobset.String,{'-','Unknown source'}))
        handles.cs_ch2to3_ch2numOrder.Enable = 'on';
        handles.cs_ch2to3_ch3numOrder.Enable = 'on';
        handles.cs_ch2to3_ch2numOrder.String = '2';
        handles.cs_ch2to3_ch3numOrder.String = num2str(2+1);
    else
        handles.cs_ch2to3_ch2numOrder.Enable = 'off';
        handles.cs_ch2to3_ch3numOrder.Enable = 'off';
        handles.cs_ch2to3_ch2numOrder.String = '';
        handles.cs_ch2to3_ch3numOrder.String = '';
    end
%     for iChan = 1:(3-1)
...
%     end
    ...
    handles.cs_ch1to2Vect.Enable = 'off';
    handles.cs_ch1to2Arrow.Enable = 'off';
    handles.cs_ch1to2Jobset.Enable = 'off';
    handles.cs_ch1to2_ch1numOrder.Enable = 'off';
    handles.cs_ch1to2_ch2numOrder.Enable = 'off';
    handles.cs_ch1to2_ch1numOrder.String = ''; %CCC 2023.08.09
    handles.cs_ch1to2_ch2numOrder.String = '';

    handles.cs_ch1to3Vect.Enable = 'off';
    handles.cs_ch1to3Arrow.Enable = 'off';
    handles.cs_ch1to3Jobset.Enable = 'off';
    handles.cs_ch1to3_ch1numOrder.Enable = 'off';
    handles.cs_ch1to3_ch3numOrder.Enable = 'off';
    handles.cs_ch1to3_ch1numOrder.String = ''; %CCC 2023.08.09
    handles.cs_ch1to3_ch3numOrder.String = '';

    handles.cs_ch2to3Vect.Enable = 'off';
    handles.cs_ch2to3Arrow.Enable = 'off';
    handles.cs_ch2to3Jobset.Enable = 'off';
    handles.cs_ch2to3_ch2numOrder.Enable = 'off';
    handles.cs_ch2to3_ch3numOrder.Enable = 'off';
    handles.cs_ch2to3_ch2numOrder.String = ''; %CCC 2023.08.09
    handles.cs_ch2to3_ch3numOrder.String = '';
...
function ch1to2CB(hObj,event)
  ...
  handles.cs_ch1to2Jobset.String = file;
  file = fullfile(path,file);
  jobset.options.chrShift.jobset{1,2} = file;
  handles.cs_ch1to2_ch1numOrder.Enable = 'on';
  handles.cs_ch1to2_ch1numOrder.String = jobset.options.chrShift.chanOrder{1,2}(1);
  handles.cs_ch1to2_ch2numOrder.Enable = 'on';
  handles.cs_ch1to2_ch2numOrder.String = jobset.options.chrShift.chanOrder{1,2}(2);
end
...
function ch1to3CB(hObj,event)
  ...
  handles.cs_ch1to3Jobset.String = file;
  file = fullfile(path,file);
  jobset.options.chrShift.jobset{1,3} = file;
  handles.cs_ch1to3_ch1numOrder.Enable = 'on';
  handles.cs_ch1to3_ch1numOrder.String = jobset.options.chrShift.chanOrder{1,3}(1);
  handles.cs_ch1to3_ch3numOrder.Enable = 'on';
  handles.cs_ch1to3_ch3numOrder.String = jobset.options.chrShift.chanOrder{1,3}(2);
end
...
function ch2to3CB(hObj,event)
  ...
  handles.cs_ch2to3Jobset.String = file;
  file = fullfile(path,file);
  jobset.options.chrShift.jobset{2,3} = file;
  handles.cs_ch2to3_ch2numOrder.Enable = 'on';
  handles.cs_ch2to3_ch2numOrder.String = jobset.options.chrShift.chanOrder{2,3}(1); %CCC 2023.08.09
  handles.cs_ch2to3_ch3numOrder.Enable = 'on'; %CCC 2023.08.09
  handles.cs_ch2to3_ch3numOrder.String = jobset.options.chrShift.chanOrder{2,3}(2); %CCC 2023.08.09
end
...
%       opts.autoRadiiAvgDisp = str2double(handles.autoRadiiAvgDisp.String)/60;
  ...
  if handles.chromaticShift.Value
    ...
    chrShift.chanOrder{1,2}(1) = str2double(handles.cs_ch1to2_ch1numOrder.String);
    chrShift.chanOrder{1,2}(2) = str2double(handles.cs_ch1to2_ch2numOrder.String);

    chrShift.chanOrder{1,3}(1) = str2double(handles.cs_ch1to3_ch1numOrder.String);
    chrShift.chanOrder{1,3}(2) = str2double(handles.cs_ch1to3_ch3numOrder.String);

    chrShift.chanOrder{2,3}(1) = str2double(handles.cs_ch2to3_ch2numOrder.String);
    chrShift.chanOrder{2,3}(2) = str2double(handles.cs_ch2to3_ch3numOrder.String);
  ...
  for i = 1:3
    for j = 1:3
      ...
      if chrShift.filtering
        mS = chrsFilterSpots(mS,'revert',0, ...
          'neighbourFilter',neighFilt,'intensityFilter',intFilt, ...
          'referenceChan',handles.spotDetectChNum); %CCC changed revert to 0 for poorly detected spots 2024.10.31
      end
      [result,~] = chrsCalculateChromaticShift(mS,[i j],...
          'filtered',1);
      cellResult{i,j} = result;
      cellResult{j,i} = result.*[-1 -1 -1 1 1 1];
    end
  end       
end
