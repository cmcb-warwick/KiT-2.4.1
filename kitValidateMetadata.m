...
if isfield(jobset,'metadata') && length(jobset.metadata)>=iMov && jobset.metadata{iMov}.validated
    ...
    for iChan=1:numWvs
      try
        ...
        try
          md.wavelength(iChan) = chWv.value(); %CCC 20260428
          md.wavelength(iChan) = double(md.wavelength(iChan)); %CCC 20260428
...
function updateControls(md)
  ...
  for iChan=1:length(md.wavelength)
    ...
    if md.wavelength(iChan) < 100
        hs.wavelength{iChan}.String = num2str(md.wavelength(iChan)*1000);
    else
        hs.wavelength{iChan}.String = num2str(md.wavelength(iChan)); %CCC 20260428
    end
  end
  for iCoord=1:3
