function [out1,out2,out3] = ginputColour(arg1,colour)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    elseif nargin == 2
        ...
        data = arg1;
        if (isa(data,'double')) && (size(data,2)==1)
            userPoints = 1;
            userColour = 0;
        else
            userPoints = 0;
    ...
    initialState = setupFcn(fig,crossColour);
...
function initialState = setupFcn(fig,crossColour)
...
initialState.CrossHair = createCrossHair(fig,crossColour);
...
function crossHair = createCrossHair(fig,crossColour)
...
if isWebFigureType(fig, 'UIFigure')
    for k = 1:4
        crossHair(k) = uilabel(fig, 'Visible', 'off', 'BackgroundColor', crossColour, 'HandleVisibility', 'off'); %#ok<AGROW>
    end
else
    for k = 1:4
        crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', crossColour, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
    end
end
