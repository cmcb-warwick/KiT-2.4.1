function selectedData = kitSelectData_sS(job,varargin)
% KITSELECTDATA_SS lets you select spots from a movie structure for further
% analysis.
%
%   KITSELECTDATA_SS(JOB, ...) creates a spot selection structure (sS_sel). 
%   JOB should be movieStructure. Sister pairing must be completed for JOB.
%   
%   Options, defaults in {}:
%   
...
%       through creating a sS_sel for this channel. Should have been
%       created using same movieStructure as JOB.
% 
...
for iMov = opts.startMovie:nMovs
    ...
    if size(screenSz,1) == 1
        set(fig, 'Position', [600 400 400 300]); %CCC change for Mac screen
    elseif screenSz(2,1) < 0
        set(fig, 'Position', [-1560 560 400 300]); %CCC change for work screen
    else
        set(fig, 'Position', [2200 300 400 300]); %CCC change for home screen, requires test
    end
    keepSelecting = uiconfirm(fig, movtxt, 'Keep selecting?', 'Options',{'Yes','Skip','No'}, ...
...
%%
function tempList = selectSpots(job,varargin)
...
handles.chanID = opts_more.coordSystemChannel;
