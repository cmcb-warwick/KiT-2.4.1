...
[rws cols]=size(dset.x.all);

if cols>2
    disp('Microscope.raw coordinate data: too many columns, either 2=paired sisters, or 1');
    dat=[];
    return;
end

if cols==2 %Paired sister data [xs1 xs2] etc
...
dat=[r acos(z./r) atan(y./x) NaN*ones(rws,2) abs(z)<zfilterz];
end
