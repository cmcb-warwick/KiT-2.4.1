...
%    Options, defaults in {}:-
%
%    coordChans: {[1 2 3]} or any subset of [1,2,3]. The channels for which
%                   sisterList structures will be produced.
%
%    imageChans: {[2]} or any subset of [1,2,3]. The channels which will
%                   be plotted when inspecting spots for pairing.
%
%    maxSisSep: {2.2} or distance in µm. Maximum distance between the
...
%
%    mode: {'dual'}, 'triple', 'zoom' or 'full'. Whether to show images as:
%                   - 'full'
%                     full images.
%                   - 'zoom'
...
%                   - 'triple'
%                     as 'dual', but with extra XZ projection.
%
...
%                   z-coordinate.
%
%    autoSize: 0 or {1}. Whether or not to automatically change figure
%                   size to fit screen.
%
%    Future directions:-
%
...
opts.imageChans = 2; % imaging more channels may provide more information %CCC 2022/10/11
opts.maxSisSep = 2.2; % maximum distance in µm between potential sisters being plotted
opts.mode = 'dual';
opts.plotChan = 2; % channel coordinates to be plotted for allocation %CCC 2022/10/11
opts.redo = 1; %CCC 2022/10/11
opts.subset = [];
opts.verbose = 0;
...
%     else
      [jobs{iMov},userStatus] = pairSpots(jobs{iMov},opts); %CCC edit from pairSpots (triple functionality fullscreen)
%     end
