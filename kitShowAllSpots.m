...
%    lineProfile: {0} or 1. Whether to calculate and plot line profile of spot.
%
...
for iSpot = opts.subset
    ...
    if noCoords
        plotTit = sprintf('No spot at %i',iSpot);
...
pause(0.1);%CCC addition

if opts.lineProfile
    opts.scaleMax = max(img,[],'all')*8;    
    figure(2);
    clf;
    tiledlayout('flow','TileSpacing','compact','Padding','compact');
    for iSpot = opts.subset
        ...
        opts.spotID = iSpot;
        nexttile;
        noCoords = plotLineProfile(job,img,opts);
...
if isfield(job.dataStruct{chan},'tracks')
    ...
    %chrShift = chrShift./pixelSize; % CCC - unneccessary? Is this a result of  2022/10/18
    %coords = coords + chrShift; % CCC - unneccessary? 2022/10/18
end
...
coords(:,1:2) = coords(:,1:2) - [xReg(1) yReg(1)] + [1 1];
...
plot(coords(1),coords(2),...
    'Color','r','Marker','x','MarkerSize',10) %CCC change from black to red 2022/10/18
hold off

end

function noCoords = plotLineProfile(job,img,opts)
...
if isfield(job.dataStruct{chan},'tracks')
    ...
    initCoord = job.dataStruct{chan}.initCoord(tp);
    coords = initCoord.allCoordPix(tk,1:3);
    %chrShift = chrShift./pixelSize; % CCC - unneccessary? 2022/10/18
    %coords = coords + chrShift; % CCC - unneccessary? 2022/10/18
end
...
plot((yReg(1):yReg(2)) - centrePxl(2), sum(imgCrpd,2),'c--','linewidth',2) %NB note dimensions 'switched' 
ylim([0,opts.scaleMax]);
hold on
plot((xReg(1):xReg(2)) - centrePxl(1), sum(imgCrpd,1),'m--','linewidth',2)
hold off
