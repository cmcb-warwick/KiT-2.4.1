  ...
  for i = options.subset
    switch options.exec
      case 'batch'
        ...
        batchJob{i} = batch(@kitTrackMovie, 1, {jobs{i},options.tasks}); %CCC 2022/10/11
      case 'serial'
        try
          kitLog('Tracking job %d', i);
          kitTrackMovie(jobs{i},options.tasks); %CCC 2022/10/11
...
for c = 1:length(jobset.options.coordMode) %CCC edit taken from KiT v2.1.10
  if ~strcmp(jobset.options.coordMode{c}, 'none') %CCC edit taken from KiT v2.1.10
    kitJobsetDiagnostics(jobset,c,0,fid); %CCC edit taken from KiT v2.1.10
  end %CCC edit taken from KiT v2.1.10
end %CCC edit taken from KiT v2.1.10
