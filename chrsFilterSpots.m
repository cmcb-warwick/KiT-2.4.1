...
%% Data handling
    ...
    for iChan = unique([opts.referenceChan opts.appliedChans])
      ...
      if isfield(job.dataStruct{iChan},'failed') && ~job.dataStruct{iChan}.failed
          %CCC change below 2024.10.30
        if opts.revert
            if  isfield(job.dataStruct{iChan},'rawInitCoord')
                origInitCoord{iChan} = job.dataStruct{iChan}.rawInitCoord; %if rawInitCoord is present (might be hangover from KiT 2.1 or earlier, not a structure made in KiT 2.4.0 onwards)
            elseif isfield(job.dataStruct{iChan},'rawData')
                if isfield(job.dataStruct{iChan}.rawData,'initCoord')
                    %detecting if pre-filtering has happened in KiT 2.4.0 onwards and obtaining the raw initCoord from this filtering regimen.
                    origInitCoord{iChan} = job.dataStruct{iChan}.rawData.initCoord;
                end
            else
                %if no rawInitCoord or rawData structures, then initCoord are unfiltered
                origInitCoord{iChan} = job.dataStruct{iChan}.initCoord(1);
            end
        else
            %if no opts.revert then continue with initCoord (either filtered or unfiltered)
            origInitCoord{iChan} = job.dataStruct{iChan}.initCoord(1);
        end
      else
        continue %CCC 2024.10.30 changed from RETURN, if spot detection fails in one cell then all subsequent cells are ignored with return
      end
...
%% Find which data requires filtering
    ...
    if minIntensity > 1
      ...
      maxAmps = sort(amps(:,opts.referenceChan),1,'descend','MissingPlacement','last'); %CCC 2024.10.30 for if you pre-filter (especially important for live)
